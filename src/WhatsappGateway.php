...
use Areypurnawan\WhatsappGateway\Exception\InvalidMethodException;
...
class WhatsappGateway
{
    ...
    public function sendMessage($to, $message, $extra_params = null, $extra_headers = [])
    {
        ...
        try {
            ...
            Log::info('Whatsapp Gateway Response Code: '. $this->responseCode);
            Log::info('Whatsapp Gateway Response Body: \n'. $this->response);
...
//            $this->response = $promise->wait()->getBody()->getContents();
            ...
            if ($e->hasResponse()) {
                ...
                Log::error('Whatsapp Gateway Response Code: '. $this->responseCode);
                Log::error('Whatsapp Gateway Response Body: \n'. $this->response);
            }
        }
        return $this;
    }
    ...
    {
        $default_acc = config('whatsapp-gateway.default', null);
        if ($default_acc) {
    ...
    protected function loadCredentialsFromConfig()
    {
        ...
        $config_name = 'whatsapp-gateway.' . $gateway;
    ...
    private function addCountryCode($mobile)
    {
        if (!$this->country_code) {
            $this->country_code = config('whatsapp-gateway.country_code', '62');
        }
        if (is_array($mobile)) {
